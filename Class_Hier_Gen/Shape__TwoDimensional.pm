...
foreach my $datum ( keys %Shape__TwoDimensional ) {
    no strict "refs";
    *{$datum} = sub {
        @_==1 or confess("[ERR ] Trying to modify read-only class attribute!");
        my $either = shift;
        return ${$Shape__TwoDimensional{$datum}}; # read-only
    }
}
