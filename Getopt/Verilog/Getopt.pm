...
sub _parameter_parse {
	...
	# Second parameters
	    ...
	    if( $self->{filename_expansion}
			...
			# is a file, bug 444.
			my $filename = $self->_parse_file_arg($optdir,$param);
			if (-e $filename) {
				push @{$self->{unparsed}}, "$filename";
			} else {
...
sub file_substitute {
    ...
    my $filename = shift;
    my $out = $filename;
    while ($filename =~ /\$([A-Za-z_0-9]+)\b/g) { # this won't handle ${VAR} style!
		my $var = $1;
		if (defined $ENV{$var}) {
