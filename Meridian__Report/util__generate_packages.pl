...
sub write_derived_class {
    ...
    if( exists $href__dont_touch->{$myself} ) {
        # don't write some-module mistakenly!(safety device)
    ...
    while( <DATA> ) {
        ...
        if( 0 ) { }
            ...
            # all attribute will be empty <- dynamic attribute
